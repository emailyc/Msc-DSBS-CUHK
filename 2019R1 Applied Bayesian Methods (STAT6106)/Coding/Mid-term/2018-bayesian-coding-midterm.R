...
d.target <- function(a,mu,theta,sigma2,tau2)
{
  prod( a*dnorm(data,mu,sqrt(sigma2)) + (1-a)*dnorm(data,theta,sqrt(tau2)) ) *1/sigma2 * 1/tau2
}

d.target2 <- function(a,mu,theta,sigma2,tau2)
{
  prod( a*dnorm(data,mu,sqrt(sigma2)) + (1-a)*dnorm(data,theta,sqrt(tau2)) ) *1/sigma2 * 1/tau2
}
