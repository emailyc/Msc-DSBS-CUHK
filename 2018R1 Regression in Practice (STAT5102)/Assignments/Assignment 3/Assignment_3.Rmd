...
$$
\begin{align*}
 log(fev) = `r coef(model_d_2)[1]` + `r coef(model_d_2)[2]` \; log(Age) + `r coef(model_d_2)[2]` \; HT  \\
\end{align*}
$$
...
We are 95% confident that the true intercept of the model is between `r CI[1,1]` and `r CI[1,2]`. That is, on average, the logged value of `fev` will be somewhere between `r CI[1,1]` and `r CI[1,2]` when all other variables have values at zero.

We are 95% confident that the true elasticity of the variable `age` is between `r CI[2,1]` and `r CI[2,2]`. That is, on average, for every 1% increase in `age`, `fev` will increase by somewhere between `r scales::percent(CI[2,1])` and `r scales::percent(CI[2,1])`. 

We are 95% confident that the true coefficient of the variable `ht` is between `r CI[3,1]` and `r CI[3,2]`. That is, for every unit increase in `ht`, `fev` will increase by somewhere between `r scales::percent(CI[3,1])` and `r scales::percent(CI[3,2])`, on average.
<br />
...
y_cor_x1
```
The p-value of the Pearson correlation test is `r y_cor_x1$p.value`. There is no evidence of a linear relation between sale price and appraised land value.

```{r}
y_cor_x2 <- cor.test(HAMILTON$Y, HAMILTON$X2)
y_cor_x2
```
The p-value of the Pearson correlation test is `r y_cor_x2$p.value`. There is no evidence of a linear relation between sale price and appraised land value.<br />
...
$R^2$ of the model is `r summary(HAMILTON_model)$r.squared`, which is abnormally high. It is possible that there is a garbage predictor in the model boosting the $R^2$. 
